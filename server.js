...
server.post('/', async (request, reply) => {
  const { eventId, eventType, applicationId, timestamp, payload } = request.body;
  ...
  try {
    await producer.send({
      ...
      messages: [
        {
          key: applicationId,
          value: JSON.stringify({ eventId, eventType, applicationId, timestamp, payload: messagePayload }),
        },
      ],
    });

    server.log.info({ eventId, eventType, applicationId }, 'Event published to Kafka.');
    ...
    server.log.error({ error: error.message, eventId, eventType, applicationId }, 'Kafka publication failed.');
